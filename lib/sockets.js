...
// Join Room
  ...
  socket.on('joinGame', function(username){
    ...
    if(gameRooms.myGameRoom(socket.room).isFull){
      let initialGameInfo = gameRooms.myGameRoom(socket.room).startGame();
  ...
  socket.on('keyInformation', function(data){
    setTimeout(sendGame, 5, data);
  })
